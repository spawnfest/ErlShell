...
pp({{tuple,true,L}, _Len}, Col, Ll, M, TInd, Ind, LD, W) ->
    [ beautify(cb,"{"), pp_tag_tuple(L, Col, Ll, M, TInd, Ind, LD, W + 1), beautify(cb,"}")];

pp({{tuple,false,L}, _Len}, Col, Ll, M, TInd, Ind, LD, W) ->
    [ beautify(cb, "{"), pp_list(L, Col + 1, Ll, M, TInd, indent(1, Ind), LD, beautify(comma,","), W + 1), 
	  beautify(cb,"}")];
...
pp({{record,[{Name,NLen} | L]}, _Len}, Col, Ll, M, TInd, Ind, LD, W) ->
    [Name, beautify(cb, "{"), pp_record(L, NLen, Col, Ll, M, TInd, Ind, LD, W + NLen+1), beautify(cb,"}")];
...
pp_field({{field, Name, NameL, F}, _Len}, Col0, Ll, M, TInd, Ind0, LD, W0) ->
    ...
    {[beautify(name,Name), beautify(eq,Sep), S | pp(F, Col, Ll, M, TInd, Ind, LD, W)], Ll}. % force nl
...
pp_binary([LT,LT,S,GT,GT], Col, Ll, M, Ind, LD, W) ->
    ...
    [ beautify(bstr,"<<"),pp_binary(S, N, N, Ind), beautify(bstr,">>")].

pp_binary([BS,$, | S], N, N0, Ind) ->
    Len = length(BS) + 1,
    case N - Len of
        N1 when N1 < 0 ->
            [$\n, Ind, beautify(digits,BS), beautify(comma,",") | pp_binary(S, N0 - Len, N0, Ind)];
        N1 ->
            [beautify(digits,BS), beautify(comma,",") | pp_binary(S, N1, N0, Ind)]
    end;
...
write({{tuple, _IsTagged, L}, _}) ->
    [beautify(cb,"{"), write_list(L, beautify(comma,",")), beautify(cb,"}")];
...
write({{record, [{Name,_} | L]}, _}) ->
    [Name, beautify(cb,"{"), write_fields(L), beautify(cb,"}")];
...
write_pair({{map_pair, K, V}, _}) ->
    [write(K), beautify(arrow," => "), write(V)].
...
write_tail([], _S) ->
    [];
write_tail([E | Es], S) ->
    [beautify(comma,","), write(E) | write_tail(Es, S)];
write_tail({dots, _}, S) ->
    [S | beautify(ellipsis,"...")];
write_tail(E, S) ->
    [S | write(E)].
        ...
        0 ->
	    	...
	    	case Str andalso printable_bin(Bin, D1, Enc) of
                {true, List} when is_list(List) ->
	            	...
	            	{[ beautify(bstr,"<<"),beautify(field,S),beautify(bstr,">>")], 4 + length(S)};
			
                {false, List} when is_list(List) ->
                    S = io_lib:write_string(List, $"), %"
	            	{[beautify(bstr,"<<"),beautify(field,S),beautify(bstr,"/utf8>>")], 9 + length(S)};
			
		        {true, true, Prefix} ->
		            S = io_lib:write_string(Prefix, $"), %"
		            {[beautify(bstr,"<<"),beautify(field,S) | beautify(ellipsis,"...")++beautify(bstr,">>")], 7 + length(S)};

		        {false, true, Prefix} ->
	                S = io_lib:write_string(Prefix, $"), %"
		            {[beautify(bstr,"<<"),beautify(field,S) | beautify(bstr,"/utf8...>>")], 12 + length(S)};
		        false ->
...
print_length_record(Tuple, D, RF, RDefs, Enc, Str) ->
    ...
    {{record, [{beautify(name,Name),NameL} | L]}, list_length(L, NameL + 2)}.
